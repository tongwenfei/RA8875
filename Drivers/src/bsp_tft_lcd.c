...
void LCD_HardReset(void)
{
	...
	/* 使能 GPIO时钟 */
	RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOE,ENABLE);//使能PORTB,D,E,G以及AFIO复用功能时钟
	
	/* 配置背光GPIO为推挽输出模式 */
	GPIO_InitStructure.GPIO_Pin =GPIO_Pin_4|GPIO_Pin_5;	 //	//PORTD复用推挽输出  
...
static void LCD_FSMCConfig(void)
{
  ...
  timingRead.FSMC_AddressSetupTime = 15;
  timingRead.FSMC_AddressHoldTime = 0;
  timingRead.FSMC_DataSetupTime = 7;
